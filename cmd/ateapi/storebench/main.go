...
var (
	...
	seedWorkerNodeName  = pflag.String("seed-worker-node-name", "", "NodeName recorded on seeded workers; required by resume's AttachVolumes step, and must equal the simulator pod's node for MintCert's node check.")
...
func main() {
	...
	workerNodeName = *seedWorkerNodeName
