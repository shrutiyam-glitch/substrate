...
const atespacePrefix = "sbench"

// Overridable so seeded rows can reference REAL WorkerPool / ActorTemplate
// CRs for end-to-end lifecycle benchmarking against a fake atelet (see
// docs/dev/fake-lifecycle-testing.md). Defaults keep the historical
// store-only benchmark identities.
var (
	workerNamespace   = "storebench"
	poolNamePrefix    = "pool-"
	templateNamespace = "storebench"
	templateName      = "synthetic"
	actorNamePrefix   = "actor-"
	workerPodPrefix   = "worker-"

	// Scheduler-eligibility fields for seeded workers. AssignWorkerStep's
	// filter requires SandboxClass == template's class, STATE_ACTIVE, and
	// labels matching the template's workerSelector — zero values (the
	// store-benchmark default) make seeded workers unschedulable, which is
	// fine for store benchmarks and wrong for lifecycle ones.
	workerSandboxClass = ""
	workerNodeName     = ""
	workerActive       = false
	workerExtraLabels  = map[string]string{}
)
...
func (d *dataset) actorName(i int) string    { return fmt.Sprintf("%s%09d", actorNamePrefix, i) }
...
// poolName: with a single pool, the prefix is used verbatim so it can match
// a real WorkerPool CR name exactly (e.g. benchmark-ateom); multi-pool
// datasets get <prefix>NN and need one CR per generated name.
func (d *dataset) poolName(i int) string {
	if d.pools == 1 {
		return poolNamePrefix
	}
	return fmt.Sprintf("%s%02d", poolNamePrefix, i%d.pools)
}
func (d *dataset) workerPod(i int) string  { return fmt.Sprintf("%s%08d", workerPodPrefix, i) }
...
func benchActor(atespace, name string) *ateapipb.Actor {
	return &ateapipb.Actor{
		...
		Status: &ateapipb.ActorStatus{
			...
			// Point resume at the actor's own seeded snapshot row; without
			// this, resume falls back to the ActorTemplate golden snapshot,
			// which benchmark stores do not carry.
			LatestSnapshot: &ateapipb.ObjectRef{Atespace: atespace, Name: name},
		},
...
func benchSnapshot(atespace, name string) *ateapipb.ActorSnapshot {
	return &ateapipb.ActorSnapshot{
		...
		Status: &ateapipb.ActorSnapshotStatus{
			...
			ActorTemplateNamespace: templateNamespace,
			ActorTemplateName:      templateName,
			// Parseable but fake: the simulator stubs Restore, so the data
			// behind this URI is never fetched.
			SnapshotUri:  "gs://storebench-fake/snapshots/" + atespace + "/" + name,
...
func benchWorker(ns, pool, pod string, i int) *ateapipb.Worker {
	...
	for k, v := range workerExtraLabels {
		labels[k] = v
	}
	w := &ateapipb.Worker{
		WorkerNamespace: ns,
		...
		Labels:          labels,
		SandboxClass:    workerSandboxClass,
		NodeName:        workerNodeName,
		// sim:// carries worker identity to the atelet simulator's MintCert
		// callback (Run/Restore only convey the pod UID; see simulator's
		// mint.go). Free-form string, valid wherever a pod UID is compared.
		WorkerPodUid:    "sim://" + ns + "/" + pod,
	}
	if workerActive {
		w.State = ateapipb.Worker_STATE_ACTIVE
	}
	return w
}
...
func opActorChurn(ctx context.Context, st store.Interface, ds *dataset, keys *keyPicker, rec *recorder) {
	...
	actor.Status = &ateapipb.ActorStatus{State: ateapipb.ActorState_ACTOR_STATE_DELETING}
...
func opList(ctx context.Context, st store.Interface, ds *dataset, keys *keyPicker, rec *recorder) {
	atespace := ds.atespaceName(keys.intn(ds.atespaces))
	start := time.Now()
	_, err := st.ListActors(ctx, atespace, store.ListOptions{PageSize: 1000})
